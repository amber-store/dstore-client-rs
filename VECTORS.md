...
`github.com/amber-store/core` v0.0.8, `transport-iroh` v0.4.0, go-udiff v0.4.1, fxamacker/cbor v2.9.3, go-iroh
...
four protocol families comes from the Go libraries pinned in PORTING.md §0: dstore v0.1.9 (`wire`, `codec`,
`ticket`, `client` error types, `node` payload types, `view.IDsOf`), transport-iroh v0.4.0 `protocol`, core
v0.0.8 `amberpack`, `key`, `reference`, go-iroh v0.2.0 `key`, urfave/cli v2.27.7.
...
`refglob/`), `github.com/amber-store/core` v0.0.8, `transport-iroh` v0.4.0 `protocol`. Durations are nanoseconds
...
dstore v0.1.9 `refglob.Compile`, `Glob.Match`, `Glob.Prefix`.
...
- **Snapshots.** `clisnap` builds `github.com/amber-store/dstore/cmd/dstore` with the vectorgen build list,
  which equals dstore v0.1.9's `go.mod`. It builds with go1.26.5, `CGO_ENABLED=0` and `-trimpath`, without
  ldflags (so `version` is `dev`), into a temporary directory, and checks the binary's build information (main
  module dstore v0.1.9). It runs every case there and deletes the binary and all fixtures afterwards.
