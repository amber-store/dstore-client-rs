...
port. The generator is `tools/vectorgen`, a Go module that requires `github.com/amber-store/dstore` v0.1.10,
`github.com/amber-store/core` v0.0.9, `transport-iroh` v0.4.0, go-udiff v0.4.1, fxamacker/cbor v2.9.3, go-iroh
...
**Status: final for dstore v0.1.10.** 18 families are registered and their files are committed, together
...
- `clisnap` builds dstore v0.1.10 `cmd/dstore` with `go build -trimpath` into a temporary directory, runs every
...
four protocol families comes from the Go libraries pinned in PORTING.md §0: dstore v0.1.10 (`wire`, `codec`,
`ticket`, `client` error types, `node` payload types, `view.IDsOf`), transport-iroh v0.4.0 `protocol`, core
v0.0.9 `amberpack`, `key`, `reference`, go-iroh v0.2.0 `key`, urfave/cli v2.27.7.
...
26 s CPU, one goroutine per set). **Go:** dstore v0.1.10 `placement.NewSet`, `Set.Rank` and `Set.Owners` over all
...
`refglob/`), `github.com/amber-store/core` v0.0.9, `transport-iroh` v0.4.0 `protocol`. Durations are nanoseconds
...
dstore v0.1.10 `refglob.Compile`, `Glob.Match`, `Glob.Prefix`.
...
- **Snapshots.** `clisnap` builds `github.com/amber-store/dstore/cmd/dstore` with the vectorgen build list,
  which equals dstore v0.1.10's `go.mod`. It builds with go1.26.5, `CGO_ENABLED=0` and `-trimpath`, without
  ldflags (so `version` is `dev`), into a temporary directory, and checks the binary's build information (main
  module dstore v0.1.10). It runs every case there and deletes the binary and all fixtures afterwards.
...
- `go run ./cmd/holdlock DIR SECONDS` opens `DIR/.dstore/packstore` with core v0.0.9, which takes its flock.
