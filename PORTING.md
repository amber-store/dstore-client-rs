...
| DD-16 | On Linux hosts whose `net.core.rmem_max`/`wmem_max` are below 7 MiB, the Go binary's quic-go fork (go-iroh `internal/qng`) logs `failed to sufficiently increase receive buffer size (was: …)` through the log package when it binds a socket, unless `QUIC_GO_DISABLE_RECEIVE_BUFFER_WARNING` is set. The Rust CLI prints nothing. The clisnap snapshots and the interop harness set that variable for the Go binary, and CI raises the limits. | A transport-library log line below dstore, as DD-4; noq does not warn. |
